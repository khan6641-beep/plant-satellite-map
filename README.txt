...
6) 검색어를 입력하고 Enter를 누르면 검색된 식물 마커만 지도에 표시됩니다.
7) 검색창의 X 또는 초기화 버튼을 누르면 전체 식물 마커가 다시 표시됩니다.
8) 검색 결과를 선택하면 줌 22까지 이동합니다.
9) 줌 22에서도 완전히 같거나 매우 가까운 좌표만 작은 클러스터로 남으며, 누르면 Spiderfy 방식으로 펼쳐집니다.
...
8. 자연어·오탈자 검색
--------------------
이번 버전은 외부 AI API 없이 GitHub Pages 안에서 다음 검색을 처리합니다.

- 정확 검색: 소나무, Ulmus parvifolia, 12-L-00001
- 자연어 검색: 소나무 위치 보여줘, 참느릅나무 어디 있어?, 표찰번호 12-L-00001 찾아줘
- 조사 처리: 무궁화를 찾아줘, 소나무는 어디 있어?
- 오탈자 보정: 왕벗나무 -> 왕벚나무, Quercuus -> Quercus

검색은 먼저 정확·부분 일치를 수행하고, 결과가 없을 때만 한글·영문 편집거리 기반 보정을 수행합니다.
검색어와 식물 데이터는 외부 서버로 전송하지 않습니다.

주의
- 이 기능은 내부 직원용 1차 서비스에 적합한 규칙·유사도 기반 지능형 검색입니다.
- 문맥이 복잡한 질의, 식물 특성 설명을 통한 검색, 대화형 답변은 별도의 서버형 AI 모델이 필요합니다.
- 짧은 두 글자 검색어는 잘못된 자동 보정을 막기 위해 오탈자 보정을 수행하지 않습니다.


9. 식물 카드 대표 이미지 등록
----------------------------
식물 마커 또는 검색 결과를 선택하면 하단 정보 카드에 대표 이미지 영역이 표시됩니다.
등록 이미지가 없거나 파일을 찾지 못하면 assets/images/plant-placeholder.svg가 표시됩니다.

이미지 등록 순서
1) 이미지 파일을 assets/images/plants/ 폴더에 넣습니다.
2) data/plant-images.js를 텍스트 편집기로 엽니다.
3) 표찰번호, 국명 또는 학명 중 하나를 기준으로 이미지 정보를 등록합니다.
4) GitHub 저장소에 이미지와 plant-images.js를 함께 올립니다.
...
국명별 등록 예시
  byCommonName: {
    "무궁화": {
      src: "./assets/images/plants/무궁화.jpg",
      alt: "무궁화 꽃 대표 이미지",
      credit: "촬영: 식물관리연구실"
    }
  }
...
학명별 등록 예시
  byScientificName: {
    "Hibiscus syriacus": {
      src: "./assets/images/plants/Hibiscus-syriacus.jpg",
      alt: "Hibiscus syriacus 대표 이미지",
      credit: "촬영: 식물관리연구실"
    }
  }

이미지 선택 우선순위
  표찰번호 > 국명 > 전체 학명 > 기본 대체 이미지

권장 이미지
- JPG 또는 WebP
- 가로형 16:9 또는 4:3
- 1장당 500KB 이하 권장
- 저작권·촬영자·라이선스를 credit와 license에 기록
- 외부 URL보다 저장소 내부 상대경로 사용 권장


10. 이번 버전 배포 전 확인
--------------------------
1) MAPBOX_TOKEN.js에 pk. 공개 토큰 입력
2) node tools/test_search.js 실행
3) python tools/validate_project.py 실행
4) GitHub Pages에 전체 폴더 업로드
5) 서비스 워커 캐시가 남아 있으면 강력 새로고침 또는 사이트 데이터 삭제
6) 실제 대표 이미지 등록 후 모바일 카드에서 잘림과 저작권 문구 확인
